...
\textbf{【组合逻辑MUX——完整VHDL】}
\begin{lstlisting}[caption={if-else综合为组合逻辑MUX}]
library ieee;
...
architecture behavioral of mux_ifelse is
begin
  process(a, b, sel)
  begin
    if sel = '1' then
...
\textbf{【时序逻辑（带使能的D触发器）——完整VHDL】}
\begin{lstlisting}[caption={if+clk综合为带使能的D触发器}]
library ieee;
use ieee.std_logic_1164.all;
...
\textbf{【case综合示例——完整VHDL】}
\begin{lstlisting}[caption={case综合为并行MUX}]
library ieee;
use ieee.std_logic_1164.all;
...
\textbf{【signal vs variable 对比示例——完整VHDL】}
\begin{lstlisting}[caption={signal（<=）与variable（:=）对比}]
library ieee;
use ieee.std_logic_1164.all;
...
architecture behavioral of sig_var_demo is
  signal sig_tmp : unsigned(7 downto 0);  -- signal：process结束后更新
begin
  process(clk, rst_n)
    variable var_tmp : unsigned(7 downto 0);  -- variable：立即更新
  begin
    if rst_n = '0' then
      sig_tmp <= (others => '0');
      result  <= (others => '0');
      ...
      var_tmp := a * b;       -- := 立即赋值
      sig_tmp <= a * b;       -- <= 延迟到process结束
      result  <= var_tmp;     -- 使用variable的立即值
    end if;
  end process;
end behavioral;
\end{lstlisting}
...
\begin{lstlisting}[caption={三种典型process及其综合结果}]
library ieee;
use ieee.std_logic_1164.all;
...
begin
  -- (1) 时序process：process(clk)+rising_edge → D触发器
  process(clk)
  begin
    if rising_edge(clk) then
      q_reg <= d;
  ...
  -- (2) 时序process带异步复位：process(clk,rst) → D触发器+异步复位
  process(clk, rst_n)
  begin
    if rst_n = '0' then
      q <= '0';
  ...
  -- (3) 组合process：process(all inputs)+无clk → 组合逻辑
  process(a, b, sel)
  begin
    if sel = '1' then
      y <= a;
